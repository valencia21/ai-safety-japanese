...
const ReadingItem: React.FC<{
  ...
  return (
    <div className="flex items-center gap-4">
      <Link
        to="/$contentId"
        params={{ contentId: reading.content_id }}
...
export const SessionInformation: React.FC = () => {
  ...
  const tabSessionNumbers = activeTab?.sessionNumbers;
  ...
  const inSessionFilter = (n: number | null | undefined): boolean => {
    if (!tabSessionNumbers || tabSessionNumbers.length === 0) return true;
    return n != null && tabSessionNumbers.includes(n);
  };
  ...
  const displayedSessions = sessions.filter(s => inSessionFilter(s.session_number));
  const activeSessionData = displayedSessions.find(s => s.id === activeSession);

  const renderIntro = () => {
    if (activeTabId === 'ai_2027') {
      return (
        <>
          <h3 className="text-xl text-stone-400 mb-1">SCENARIO</h3>
          <h2 className="text-3xl font-bold text-stone-900 mb-6">AI 2027年</h2>
          <p className="text-sm font-bold mb-4">
            Daniel Kokotajlo らによる、超知能の到来を描いた未来予測シナリオの日本語版です。原文は <a href="https://ai-2027.com/" target="_blank" rel="noopener noreferrer" className="underline">ai-2027.com</a> をご覧ください。
          </p>
          <p className="text-stone-600 text-xl mt-0.5">
            2025年から2030年にかけて、AIエージェントの登場、米中軍拡競争、Agent シリーズの進化、そして二つの結末（競争／減速）までを月単位で描いた、超知能社会の具体的なシナリオです。
          </p>
        </>
      );
    }
  ...
  return (
    <div className="w-full">
      <div className="session-layout relative border-t border-stone-900">
        <div className="session-grid divide-stone-900">
          {/* Left sidebar - only visible on lg and above */}
          <div className="session-sidebar relative">
            <div>
          ...
          <div className="session-content">
            ...
            <div className="session-mobile my-4">
              {/* Intro Text */}
